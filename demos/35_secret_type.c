/* Generated by FORGE compiler — do not edit.
   All proof obligations discharged. This code is correct by construction. */

#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>
#ifndef __GNUC__
#  define __attribute__(x)
#endif

/* Monomorphized generic types */

/* Forward declarations */
uint64_t ct_select(volatile uint64_t flag __attribute__((unused)), volatile uint64_t a __attribute__((unused)), volatile uint64_t b __attribute__((unused)));
uint64_t ct_eq(volatile uint64_t a __attribute__((unused)), volatile uint64_t b __attribute__((unused)));
uint64_t mulmod(volatile uint64_t a __attribute__((unused)), volatile uint64_t b __attribute__((unused)), uint64_t q __attribute__((unused)));
uint64_t run();

uint64_t ct_select(volatile uint64_t flag __attribute__((unused)), volatile uint64_t a __attribute__((unused)), volatile uint64_t b __attribute__((unused))) {
  volatile uint64_t mask = (0 - flag);
  volatile uint64_t neg_mask = (~mask);
  return ((mask & a) | (neg_mask & b));
}

uint64_t ct_eq(volatile uint64_t a __attribute__((unused)), volatile uint64_t b __attribute__((unused))) {
  volatile uint64_t diff = (a ^ b);
  volatile uint64_t neg_diff = (0 - diff);
  volatile uint64_t x = (diff | neg_diff);
  return (1 - (x >> 63));
}

uint64_t mulmod(volatile uint64_t a __attribute__((unused)), volatile uint64_t b __attribute__((unused)), uint64_t q __attribute__((unused))) {
  return ((a * b) % q);
}

uint64_t run() {
  volatile uint64_t a = 7;
  volatile uint64_t b = 11;
  uint64_t q = 17;
  volatile uint64_t product = mulmod(a, b, q);
  volatile uint64_t selected = ct_select(1, a, b);
  volatile uint64_t eq_sum = (ct_eq(a, b) + ct_eq(a, a));
  return (((selected + product) + eq_sum) & 255);
}

