"""Forge-native SiLU (Swish) — GPU validation harness.

Kernel: silu_kernel(inp: span<f32>, out: span<f32>, n: u64)
        y = x / (1 + expf(-x))
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1130_forge_silu.cubin'))
...
print("FORGE: silu_kernel - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for N in sizes:
    ...
    fn = ctypes.c_void_p()
    ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"silu_kernel"))

    np.random.seed(N)
    x = (np.random.randn(N).astype(np.float32) * 4.0)
    ...
    y_ref = silu_cpu(x).astype(np.float32)
