...
int main();
__global__ void layernorm_rowwise(forge_span_f32_t inp __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused)));
int main();
...
__global__ void layernorm_rowwise(forge_span_f32_t inp __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused))) {
  ...
  float row_sum __attribute__((unused)) = smem[0ULL];
  __syncthreads();
  ...
  smem[tid] = (d * d);
  __syncthreads();
  uint64_t s2 __attribute__((unused)) = 128ULL;
  {
    while ((s2 > 0ULL)) {
      if ((tid < s2)) {
  ...
  float row_sumsq __attribute__((unused)) = smem[0ULL];
  float var __attribute__((unused)) = (row_sumsq / n_f);
  if ((tid < n_cols)) {
    float inv_std __attribute__((unused)) = rsqrtf((var + eps));
    out.data[(base + tid)] = (d * inv_std);
