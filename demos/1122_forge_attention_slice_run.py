"""Forge-native single-thread attention slice — GPU validation.

Kernel: attention_slice(Q, K, V, O, seq_len, d)
        out[i] = softmax(Q[i] @ K^T / sqrt(d)) @ V    one thread per query row.
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1122_forge_attention_slice.cubin'))
...
cases = [(8, 16), (16, 32), (32, 64), (64, 32), (128, 16)]   # seq_len, d
print("FORGE: attention_slice - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for (S, D) in cases:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"attention_slice"))
