"""Forge-native fp16_rsqrt — GPU validation harness.

Kernel: fp16_rsqrt(inp: span<u16>, out: span<u16>, n: u64)
        u16 (fp16 bit pattern) -> f32 -> rsqrtf -> u16 (fp16 bit pattern)
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1124_forge_fp16_rsqrt.cubin'))
...
print("FORGE: fp16_rsqrt - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for N in sizes:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"fp16_rsqrt"))
    np.random.seed(N)
    x_f32 = (np.random.rand(N).astype(np.float32) * 9.0 + 0.5)  # in [0.5, 9.5], avoids 0
    x_f16 = x_f32.astype(np.float16)
    x_u16 = x_f16.view(np.uint16)
    ...
    y_gpu = y_u16_gpu.view(np.float16).astype(np.float32)
    y_ref = (1.0 / np.sqrt(x_f16.astype(np.float32))).astype(np.float16).astype(np.float32)
    err = np.abs(y_gpu - y_ref); max_err, mean_err = float(err.max()), float(err.mean())
    # rsqrt.approx + fp16 round-trip — fp16 ULP at 1.0 is ~1e-3
