/* Generated by FORGE compiler — do not edit.
   All proof obligations discharged. This code is correct by construction. */

#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>
#ifndef __GNUC__
#  define __attribute__(x)
#endif

/* span<T> typedefs — fat pointers with proven bounds */
typedef struct { uint64_t* data; uintptr_t len; } forge_span_u64_t;

/* Forward declarations */
uint64_t count_in_range(forge_span_u64_t s __attribute__((unused)), uint64_t lo __attribute__((unused)), uint64_t hi __attribute__((unused)), uint64_t threshold __attribute__((unused)));
int main();

uint64_t count_in_range(forge_span_u64_t s __attribute__((unused)), uint64_t lo __attribute__((unused)), uint64_t hi __attribute__((unused)), uint64_t threshold __attribute__((unused))) {
  uint64_t count __attribute__((unused)) = 0ULL;
  {
    for (uint64_t i = (uint64_t)(lo); i < (uint64_t)(hi); i++) {
      if ((s.data[i] > threshold)) {
        count = (count + 1ULL);

      }
    }

  }
  return count;
}

int main() {
  return (int)(0ULL);

}

