...
__global__ void conv2d_3x3(uint32_t* __restrict__ inp, uint64_t inp_len, uint32_t* __restrict__ out, uint64_t out_len, uint32_t width, uint32_t height) {
        ...
        if ((y < 15U)) {
          if ((col > 0U)) {
            if (((col + 1U) < width)) {
              if ((row > 0U)) {
                if (((row + 1U) < height)) {
                  uint32_t nw = smem[(s_idx - 17U)];
                  uint32_t n = smem[(s_idx - 16U)];
                  uint32_t ne = smem[(s_idx - 15U)];
                  uint32_t w = smem[(s_idx - 1U)];
                  uint32_t c = smem[s_idx];
                  uint32_t e = smem[(s_idx + 1U)];
                  uint32_t sw = smem[(s_idx + 15U)];
                  uint32_t s = smem[(s_idx + 16U)];
                  uint32_t se = smem[(s_idx + 17U)];
                  uint32_t val = ((((((((nw + (2U * n)) + ne) + (2U * w)) + (4U * c)) + (2U * e)) + sw) + (2U * s)) + se);
