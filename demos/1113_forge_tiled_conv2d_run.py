"""FORGE57-60 GPU harness — 3x3 weighted convolution (Gaussian blur)."""
import ctypes
import numpy as np
...
CUBIN = r"C:\users\kraken\forge\demos\1113_forge_tiled_conv2d.cubin"
...
def lcg(n, seed=0xC0FFEEAB):
    ...
    for i in range(n):
        ...
        arr[i] = s & 0xFF  # small values to avoid overflow
    return arr

def cpu_conv2d(inp, W, H):
    ...
    for r in range(1, H - 1):
        for c in range(1, W - 1):
            ...
            for dy in range(-1, 2):
                for dx in range(-1, 2):
                    s = np.uint32(s + np.uint32(K[dy + 1][dx + 1]) * np.uint32(inp[(r + dy) * W + (c + dx)]))
            out[r * W + c] = s
    return out
...
for (W, H) in shapes:
    for pname, pfn in patterns:
        ...
        fn = ctypes.c_void_p()
        chk(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"conv2d_3x3"))
        ...
        expected = cpu_conv2d(inp, W, H)

        # Check tile-interior AND global-interior points
        mis = 0
        n_checked = 0
        for byi in range(by):
            for bxi in range(bx):
