...
__global__ void swiglu(forge_span_f32_t gate __attribute__((unused)), forge_span_f32_t up __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n __attribute__((unused)));
int main();

__global__ void swiglu(forge_span_f32_t gate __attribute__((unused)), forge_span_f32_t up __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n __attribute__((unused))) {
  uint64_t gid __attribute__((unused)) = ((blockIdx_x * ((uint64_t)blockDim_x)) + ((uint64_t)threadIdx_x));
  if ((gid < n)) {
    float g __attribute__((unused)) = gate.data[gid];
    float u __attribute__((unused)) = up.data[gid];
    float neg_g __attribute__((unused)) = (0 - g);
    float e __attribute__((unused)) = expf(neg_g);
    float sigmoid __attribute__((unused)) = (1 / (1 + e));
    out.data[gid] = ((g * sigmoid) * u);
