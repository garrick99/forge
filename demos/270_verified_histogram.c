/* Generated by FORGE compiler — do not edit.
   All proof obligations discharged. This code is correct by construction. */

#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>
#ifndef __GNUC__
#  define __attribute__(x)
#endif

/* span<T> typedefs — fat pointers with proven bounds */
typedef struct { uint64_t* data; uintptr_t len; } forge_span_u64_t;

/* Forward declarations */
void histogram(forge_span_u64_t s __attribute__((unused)), uint64_t n __attribute__((unused)), forge_span_u64_t hist __attribute__((unused)), uint64_t buckets __attribute__((unused)));
int main();

void histogram(forge_span_u64_t s __attribute__((unused)), uint64_t n __attribute__((unused)), forge_span_u64_t hist __attribute__((unused)), uint64_t buckets __attribute__((unused))) {
  {
    for (uint64_t i = 0; i < (uint64_t)(n); i++) {
      uint64_t bucket __attribute__((unused)) = (s.data[i] % buckets);
      hist.data[bucket] = (hist.data[bucket] + 1);
    }

  }
}

int main() {
  return (int)(0);

}

