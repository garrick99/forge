"""Forge-native softmax_warp (single-warp, n_cols<=32).
Kernel: softmax_warp(inp, out, n_cols) — standard rowwise softmax via warp-shuffle.
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1134_forge_softmax_warp.cubin'))
...
print("FORGE: softmax_warp - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for (R, C) in shapes:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"softmax_warp"))
    ...
    y_ref = softmax_cpu(x).astype(np.float32)
