...
__device__ uint32_t map_chain_u32(uint32_t av, uint32_t bv, uint32_t cv) {
  uint32_t tmp = ((av * 3U) + 7U);
  return ((tmp ^ bv) + cv);
}

__global__ void vec_map_compose(uint32_t* __restrict__ a, uint64_t a_len, uint32_t* __restrict__ b, uint64_t b_len, uint32_t* __restrict__ c, uint64_t c_len, uint32_t* __restrict__ out, uint64_t out_len, uint32_t n) {
