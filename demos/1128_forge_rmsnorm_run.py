"""Forge-native row-wise RMSNorm - GPU validation harness.

Kernel: rmsnorm_rowwise(inp: span<f32>, out: span<f32>, n_cols: u64)
        y[i,j] = x[i,j] * rsqrt(mean(x[i,:]^2) + eps)
Block-per-row, BLOCK=256, n_cols<=256.
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1128_forge_rmsnorm.cubin'))
...
print("FORGE: rmsnorm_rowwise - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for (R, C) in shapes:
    ...
    fn = ctypes.c_void_p()
    ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"rmsnorm_rowwise"))
