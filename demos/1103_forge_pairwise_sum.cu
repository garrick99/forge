...
__global__ void vec_pairwise_sum(uint32_t* __restrict__ a, uint64_t a_len, uint32_t* __restrict__ out, uint64_t out_len, uint32_t n, uint32_t n_minus_1) {
  uint32_t tid = ((blockIdx.x * blockDim.x) + threadIdx.x);
  if ((tid < n_minus_1)) {
    uint32_t i_next = (tid + 1U);
    uint32_t v0 = a[tid];
    uint32_t v1 = a[i_next];
    out[tid] = (v0 + v1);
  }
}
