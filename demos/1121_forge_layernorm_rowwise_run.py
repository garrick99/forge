"""Forge-native row-wise LayerNorm (no affine) - GPU validation harness.

Kernel: layernorm_rowwise(inp: span<f32>, out: span<f32>, n_cols: u64)
        y = (x - mean) / sqrt(var + eps)
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1121_forge_layernorm_rowwise.cubin'))
...
print("FORGE: layernorm_rowwise - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for (R, C) in shapes:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"layernorm_rowwise"))
    ...
    y_ref = layernorm_cpu(x).astype(np.float32)
