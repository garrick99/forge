...
int main();
__global__ void softmax_rowwise(forge_span_f32_t inp __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused)));
int main();
...
__global__ void softmax_rowwise(forge_span_f32_t inp __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused))) {
  ...
  uint64_t s __attribute__((unused)) = 128ULL;
  {
    while ((s > 0ULL)) {
      if ((tid < s)) {
        float a __attribute__((unused)) = smem[tid];
        float b __attribute__((unused)) = smem[(tid + s)];
        if ((b > a)) {
          smem[tid] = b;
  ...
  float row_max __attribute__((unused)) = smem[0ULL];
  __syncthreads();
  ...
  if ((tid < n_cols)) {
    /* assume erased */;
    out.data[(base + tid)] = (ex / smem[0ULL]);
...
/* ---- FORGE ASSUMPTION AUDIT LOG ----
   Total assumptions: 1
   [ASSUME] demos/1120_forge_softmax_rowwise.fg:87  (> (select smem 0) 0)
