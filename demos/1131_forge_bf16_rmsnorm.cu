...
int main();
__global__ void rmsnorm_bf16(forge_span_u16_t inp __attribute__((unused)), forge_span_u16_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused)));
int main();
...
__global__ void rmsnorm_bf16(forge_span_u16_t inp __attribute__((unused)), forge_span_u16_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused))) {
  ...
  if ((tid < n_cols)) {
    x = __bfloat162float(inp.data[(base + tid)]);

  }
  smem[tid] = (x * x);
  __syncthreads();
  uint64_t s __attribute__((unused)) = 128ULL;
  {
    while ((s > 0ULL)) {
      if ((tid < s)) {
  ...
  float ms __attribute__((unused)) = (smem[0ULL] / n_f);
  float inv_rms __attribute__((unused)) = rsqrtf((ms + eps));
  if ((tid < n_cols)) {
    out.data[(base + tid)] = __float2bfloat16((x * inv_rms));
