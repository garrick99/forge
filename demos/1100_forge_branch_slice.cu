...
__device__ uint32_t diff_scaled_u32(uint32_t a, uint32_t b) {
  if ((a > b)) {
    return ((a - b) * 2U);
  } else {
    return ((b - a) * 3U);
...
__global__ void vec_diff_branch(uint32_t* __restrict__ a, uint64_t a_len, uint32_t* __restrict__ b, uint64_t b_len, uint32_t* __restrict__ out, uint64_t out_len, uint32_t n) {
