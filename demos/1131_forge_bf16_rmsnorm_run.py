"""Forge-native bf16 row-wise RMSNorm — GPU validation harness.
Kernel: rmsnorm_bf16(inp: span<u16>, out: span<u16>, n_cols: u64), BLOCK=256.
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1131_forge_bf16_rmsnorm.cubin'))
...
print("FORGE: rmsnorm_bf16 - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for (R, C) in shapes:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"rmsnorm_bf16"))
    np.random.seed(R * 31 + C)
    x_f32 = np.random.randn(R, C).astype(np.float32)
    x_u16 = f32_to_bf16(x_f32); x_back = bf16_to_f32(x_u16)
    y_u16_gpu = np.zeros((R, C), dtype=np.uint16)
    nbytes = R * C * 2
    ...
    y_ref = bf16_to_f32(f32_to_bf16(rmsnorm_cpu(x_back).astype(np.float32)))
