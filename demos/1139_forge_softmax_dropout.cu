...
__global__ void softmax_dropout(forge_span_f32_t inp __attribute__((unused)), forge_span_u32_t mask __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused)), float keep_scale __attribute__((unused)));
int main();
...
__global__ void softmax_dropout(forge_span_f32_t inp __attribute__((unused)), forge_span_u32_t mask __attribute__((unused)), forge_span_f32_t out __attribute__((unused)), uint64_t n_cols __attribute__((unused)), float keep_scale __attribute__((unused))) {
  ...
  if ((lane < n_cols)) {
    /* assume erased */;
    uint32_t m_keep __attribute__((unused)) = mask.data[(base + lane)];
    float p __attribute__((unused)) = (ex / denom);
    float kept;
    if ((m_keep == 0ULL)) {
      kept = 0;
    } else {
...
/* ---- FORGE ASSUMPTION AUDIT LOG ----
   Total assumptions: 4
   [ASSUME] demos/1139_forge_softmax_dropout.fg:38  (< (+ base lane) inp__len)
             "blockIdx<gridDim + lane<n_cols + gridDim*n_cols<=inp.len"
   [ASSUME] demos/1139_forge_softmax_dropout.fg:49  (> denom 0)
             "argmax lane contributes p=exp(0)=1"
   [ASSUME] demos/1139_forge_softmax_dropout.fg:53  (< (+ base lane) mask__len)
             "mirrors inp bounds"
   [ASSUME] demos/1139_forge_softmax_dropout.fg:58  (< (+ base lane) out__len)
             "mirrors inp bounds"
   ---- END AUDIT LOG ---- */
