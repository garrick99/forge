...
__global__ void matmul_kloop(uint32_t* __restrict__ a, uint64_t a_len, uint32_t* __restrict__ b, uint64_t b_len, uint32_t* __restrict__ c, uint64_t c_len, uint32_t m, uint32_t n, uint32_t k) {
  ...
  while ((kt < num_k_tiles)) {
    uint32_t k_base = (kt * 16U);
    if ((row < m)) {
      uint32_t a_col = (k_base + x);
      if ((a_col < k)) {
        uint32_t a_idx = ((row * k) + a_col);
        if ((a_idx < a_len)) {
    ...
    if ((col < n)) {
      uint32_t b_row = (k_base + y);
      if ((b_row < k)) {
        uint32_t b_idx = ((b_row * n) + col);
        if ((b_idx < b_len)) {
