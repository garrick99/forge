"""Forge-native bf16 SiLU — GPU validation harness.
Kernel: silu_bf16(inp: span<u16>, out: span<u16>, n: u64)
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1133_forge_bf16_silu.cubin'))
...
def silu_cpu(x): return x / (1.0 + np.exp(-x))


sizes = [16, 256, 1024, 4096, 65536]
print("FORGE: silu_bf16 - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for N in sizes:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"silu_bf16"))
    np.random.seed(N)
    x_f32 = np.random.randn(N).astype(np.float32) * 4.0
    ...
    y_ref = bf16_to_f32(f32_to_bf16(silu_cpu(x_back).astype(np.float32)))
