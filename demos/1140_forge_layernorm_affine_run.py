"""Forge-native LayerNorm with affine (gamma, beta) - GPU validation harness.

Kernel: layernorm_affine(inp, out, gamma, beta, n_cols)
        y = ((x - mean) / sqrt(var + eps)) * gamma + beta
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1140_forge_layernorm_affine.cubin'))
...
shapes = [(4, 16), (8, 64), (16, 128), (32, 256), (128, 256)]

print("FORGE: layernorm_affine - proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for (R, C) in shapes:
    ...
    fn = ctypes.c_void_p(); ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"layernorm_affine"))

    np.random.seed(R * 31 + C)
    x = np.random.randn(R, C).astype(np.float32)
    gamma = (np.random.rand(C).astype(np.float32) + 0.5)  # in [0.5, 1.5]
    ...
    p_in_data = ctypes.c_uint64(d_in.value); p_in_len = ctypes.c_uint64(R * C)
    p_out_data = ctypes.c_uint64(d_out.value); p_out_len = ctypes.c_uint64(R * C)
    p_g_data = ctypes.c_uint64(d_g.value); p_g_len = ctypes.c_uint64(C)
    p_b_data = ctypes.c_uint64(d_b.value); p_b_len = ctypes.c_uint64(C)
    ...
    params = (ctypes.c_void_p * 9)(
        ...
        ctypes.cast(ctypes.pointer(p_b_data), ctypes.c_void_p),
        ctypes.cast(ctypes.pointer(p_b_len), ctypes.c_void_p),
