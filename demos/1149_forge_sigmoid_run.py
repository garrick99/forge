"""Forge-native sigmoid — GPU validation harness.

Compares forge-emitted cubin output against numpy reference.
Kernel: sigmoid_kernel(inp: span<f32>, out: span<f32>, n: u64)
        y = 1 / (1 + expf(-x))
"""
import ctypes, os
import numpy as np

CUBIN = os.path.abspath(os.path.join(os.path.dirname(__file__), '1149_forge_sigmoid.cubin'))
...
print("FORGE: sigmoid_kernel — proof-verified .fg -> CUDA C -> PTX -> SASS")
...
for N in sizes:
    ...
    fn = ctypes.c_void_p()
    ck(cuda.cuModuleGetFunction(ctypes.byref(fn), mod, b"sigmoid_kernel"))

    np.random.seed(N)
    x = (np.random.randn(N).astype(np.float32) * 4.0)  # range ~[-12, 12]
