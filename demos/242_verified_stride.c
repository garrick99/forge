/* Generated by FORGE compiler — do not edit.
   All proof obligations discharged. This code is correct by construction. */

#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>
#ifndef __GNUC__
#  define __attribute__(x)
#endif

/* span<T> typedefs — fat pointers with proven bounds */
typedef struct { uint64_t* data; uintptr_t len; } forge_span_u64_t;

/* Forward declarations */
void fill_stride(forge_span_u64_t s __attribute__((unused)), uint64_t n __attribute__((unused)), uint64_t stride __attribute__((unused)), uint64_t count __attribute__((unused)), uint64_t val __attribute__((unused)));
int main();

void fill_stride(forge_span_u64_t s __attribute__((unused)), uint64_t n __attribute__((unused)), uint64_t stride __attribute__((unused)), uint64_t count __attribute__((unused)), uint64_t val __attribute__((unused))) {
  {
    for (uint64_t i = 0; i < (uint64_t)(count); i++) {
      s.data[(i * stride)] = val;
    }

  }
}

int main() {
  return (int)(0ULL);

}

