/* Generated by FORGE compiler — do not edit.
   All proof obligations discharged. This code is correct by construction. */

#include <stdint.h>
#include <stdbool.h>
#include <stdlib.h>
#ifndef __GNUC__
#  define __attribute__(x)
#endif

/* span<T> typedefs — fat pointers with proven bounds */
typedef struct { uint64_t* data; uintptr_t len; } forge_span_u64_t;

/* Forward declarations */
void radix_sort_pass(forge_span_u64_t input __attribute__((unused)), forge_span_u64_t output __attribute__((unused)), uint64_t n __attribute__((unused)), forge_span_u64_t counts __attribute__((unused)), uint64_t shift __attribute__((unused)));
int main();

void radix_sort_pass(forge_span_u64_t input __attribute__((unused)), forge_span_u64_t output __attribute__((unused)), uint64_t n __attribute__((unused)), forge_span_u64_t counts __attribute__((unused)), uint64_t shift __attribute__((unused))) {
  uint64_t c __attribute__((unused)) = 0ULL;
  {
    while ((c < 256ULL)) {
      counts.data[c] = 0ULL;
      c = (c + 1ULL);
    }

  }
  uint64_t i __attribute__((unused)) = 0ULL;
  {
    while ((i < n)) {
      uint64_t digit __attribute__((unused)) = ((input.data[i] / shift) % 256ULL);
      counts.data[digit] = (counts.data[digit] + 1ULL);
      i = (i + 1ULL);
    }

  }
  uint64_t sum __attribute__((unused)) = 0ULL;
  uint64_t d __attribute__((unused)) = 0ULL;
  {
    while ((d < 256ULL)) {
      uint64_t tmp __attribute__((unused)) = counts.data[d];
      counts.data[d] = sum;
      if (((sum + tmp) <= n)) {
        sum = (sum + tmp);

      }
      d = (d + 1ULL);
    }

  }
  uint64_t j __attribute__((unused)) = 0ULL;
  {
    while ((j < n)) {
      uint64_t digit2 __attribute__((unused)) = ((input.data[j] / shift) % 256ULL);
      uint64_t pos __attribute__((unused)) = counts.data[digit2];
      if ((pos < n)) {
        output.data[pos] = input.data[j];
        counts.data[digit2] = (pos + 1ULL);

      }
      j = (j + 1ULL);
    }

  }
}

int main() {
  return (int)(0ULL);

}

