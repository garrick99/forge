...
let rec lower_expr st e : string =
  ...
  | EBinop (op, l, r_e) ->
      ...
      let instr = match op with
        | Add    -> Printf.sprintf "add.%s %s, %s, %s;" pfx dst rl rr
        | Sub    -> Printf.sprintf "sub.%s %s, %s, %s;" pfx dst rl rr
        | Mul    -> Printf.sprintf "mul.lo.%s %s, %s, %s;" pfx dst rl rr
        | Div    -> Printf.sprintf "div.%s %s, %s, %s;" pfx dst rl rr
        | Mod    -> Printf.sprintf "rem.%s %s, %s, %s;" pfx dst rl rr
        | BitAnd -> Printf.sprintf "and.b%d %s, %s, %s;" (sizeof_rty rty*8) dst rl rr
        | BitOr  -> Printf.sprintf "or.b%d %s, %s, %s;"  (sizeof_rty rty*8) dst rl rr
        | BitXor -> Printf.sprintf "xor.b%d %s, %s, %s;" (sizeof_rty rty*8) dst rl rr
        | Shl    -> Printf.sprintf "shl.b%d %s, %s, %s;" (sizeof_rty rty*8) dst rl rr
        | Shr    ->
            (* arithmetic vs logical shift: use shr for unsigned, shr for signed *)
            (match rty with
             | S16|S32|S64 ->
                 Printf.sprintf "shr.s%d %s, %s, %s;" (sizeof_rty rty*8) dst rl rr
             | _ ->
                 Printf.sprintf "shr.u%d %s, %s, %s;" (sizeof_rty rty*8) dst rl rr)
        (* Comparisons → .pred register + select into integer *)
        | Eq  | Ne  | Lt  | Le  | Gt  | Ge ->
            let pred = fresh_reg st Pred in
            let cpfx = cmp_pfx (rty_of_expr l) in
            let op_s = match op with
              | Eq -> "eq" | Ne -> "ne" | Lt -> "lt"
              | Le -> "le" | Gt -> "gt" | Ge -> "ge" | _ -> "eq" in
            emit st (Printf.sprintf "setp.%s.%s %s, %s, %s;" op_s cpfx pred rl rr);
            (* Return bool as u32: 1 if true, 0 if false *)
            let r32 = fresh_reg st U32 in
            emit st (Printf.sprintf "selp.u32 %s, 1, 0, %s;" r32 pred);
            r32
        | And | Or ->
            let pfx2 = "b32" in
            let op2 = match op with And -> "and" | _ -> "or" in
            Printf.sprintf "%s.%s %s, %s, %s;" op2 pfx2 dst rl rr
        | Implies | Iff -> Printf.sprintf "mov.u32 %s, 1; // logical not in PTX" dst
      in
      (match op with Eq|Ne|Lt|Le|Gt|Ge -> () | _ -> emit st instr);
      (match op with Eq|Ne|Lt|Le|Gt|Ge -> dst | _ -> dst)
  ...
  | EIndex (arr, idx) ->
      ...
      (* Widen idx to 64-bit for address arithmetic *)
      let ridx64 = fresh_reg st U64 in
      emit st (Printf.sprintf "cvt.u64.u32 %s, %s;" ridx64 ridx);
      ...
      let bits = sizeof_rty elem_rty * 8 in
      emit st (Printf.sprintf "ld.global%s.u%d %s, [%s];" cv bits dst addr);
      dst
  ...
  | EIf (cond, then_, else_opt) ->
      ...
      let pred = fresh_reg st Pred in
      emit st (Printf.sprintf "setp.ne.u32 %s, %s, 0;" pred rc);
      let rty  = rty_of_expr e in
      let dst  = fresh_reg st rty in
      let lbl_then = Printf.sprintf "%s_then_%d" st.fn_name st.counter in
      let lbl_else = Printf.sprintf "%s_else_%d" st.fn_name st.counter in
      let lbl_end  = Printf.sprintf "%s_end_%d"  st.fn_name st.counter in
...
and lower_assign st lhs rv =
  ...
  | EIndex (arr, idx) ->
      ...
      let bits = sizeof_rty elem_rty * 8 in
      emit st (Printf.sprintf "st.global%s.u%d [%s], %s;" cs bits addr rv)
